...
from bor.verify import verify_primary_proof_dict, HashMismatchError, replay_master
...
def _build_primary_json(tmp_path: Path, S0=3, C={"offset": 2}, V="v1.0"):
    ...
    p.write_text(json.dumps(primary, separators=(",", ":"), sort_keys=True), encoding="utf-8")
    return primary, p
...
def test_p3_verify_success(tmp_path):
    ...
    report = verify_primary_proof_dict(primary, S0=3, C={"offset": 2}, V="v1.0", stages=[add, square])
...
def test_p3_verify_mismatch(tmp_path):
    ...
    primary, _ = _build_primary_json(tmp_path)
    try:
        verify_primary_proof_dict(primary, S0=4, C={"offset": 2}, V="v1.0", stages=[add, square])
...
def test_p3_verify_config_sensitivity(tmp_path):
    ...
    try:
        verify_primary_proof_dict(primary, S0=5, C={"offset": 999}, V="v1.0", stages=[add, square])
        assert False, "Expected mismatch"
    except HashMismatchError as e:
        assert '"verified": false' in str(e) or '"verified":false' in str(e)
...
def test_p3_verify_version_sensitivity(tmp_path):
    ...
    try:
        verify_primary_proof_dict(primary, S0=5, C={"offset": 2}, V="v2.0", stages=[add, square])
        assert False, "Expected mismatch"
    except HashMismatchError as e:
        assert '"verified": false' in str(e) or '"verified":false' in str(e)
...
def test_p3_verify_step_order_matters(tmp_path):
    ...
    try:
        ...
        verify_primary_proof_dict(primary, S0=3, C={"offset": 2}, V="v1.0", stages=[square, add])
        assert False, "Expected mismatch"
    except HashMismatchError as e:
        assert '"verified": false' in str(e) or '"verified":false' in str(e)
